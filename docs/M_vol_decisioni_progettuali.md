...
Su 7 giorni di osservazione si ottengono $7 \times 24 = 168$ bucket per host.

* Bucket più piccoli (es. 5 minuti) generano $\sim 2000$ campioni, ma sono sensibili a picchi istantanei legittimi che inquinano la statistica.
* Bucket più grandi (es. 24 ore) generano $\sim 7$ campioni, troppo pochi per una buona stima della dispersione.
...
Una baseline calcolata sull'intero campione delle 168 ore presenta un problema, gli host aziendali hanno una **stagionalità settimanale** (orario lavorativo vs notte, feriali vs weekend). Su 168 bucket, un host tipico ha circa 45 ore di attività lavorativa e $\sim 123$ ore di traffico nullo. La mediana viene così dominata dalla maggioranza inattiva, tendendo a valori molto bassi.
...
|-----------|-------------|-------------------------------|
| `feriale_lavorativo` | lun-ven, ore 9-17 | $\sim 45$ |
| `feriale_fuoriorario` | lun-ven, ore 0-8 e 18-23 | $\sim 75$ |
| `weekend` | sab-dom, tutte le ore | $\sim 48$ |
...
* **Host quasi inattivo**: mediana $\approx 100$ byte/ora, $MAD_{eff} \approx 10$ byte; se l'host invia 50 KB (banale traffico web), $Z \approx 5000$. L'allarme scatterebbe per qualunque attività diversa dal traffico occasionale.
* **Auto-update/backup occasionale**: mediana $1$ MB/ora, $MAD_{eff} \approx 200$ KB; se l'host trasferisce 20 MB (aggiornamento o sincronizzazione cloud), $Z \approx 95$. Lo scostamento statistico è reale, ma 20 MB non costituiscono un'esfiltrazione significativa.
...
2. **$V_{out} > \tilde{V} (mediana)$** garantisce la direzionalità, cioè la metrica scatta solo per scostamenti **in eccesso**, non per riduzioni del traffico.
...
\boxed{
\begin{aligned}
&MAD_{eff} = \max(MAD,\; 0.10 \cdot \tilde{V}) \\[4pt]
&Z_{robusto} = \frac{V_{out} - \tilde{V}}{MAD_{eff}} \\[4pt]
&M_{vol} = 1 \iff Z_{robusto} > 3 \;\land\; V_{out} > \tilde{V} \;\land\; V_{out} > V_{min}
\end{aligned}
}
$$
