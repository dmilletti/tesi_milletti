...
* **Obiettivo:** Rilevare tentativi di mascheramento delle comunicazioni verso l'esterno, verificando la presenza del parametro SNI (Server Name Indication) esclusivamente nelle connessioni dirette a servizi web (HTTPS). Poiché i browser e le applicazioni legittime includono sempre il nome del sito di destinazione in chiaro durante la connessione iniziale, l'assenza volontaria di questo dato (ad esempio, quando un malware tenta di contattare direttamente un indirizzo IP numerico per nascondersi) rivela un'evidente anomalia strutturale, tipica di script malevoli o strumenti di hacking.
* **Metodologia di estrazione:** L'ispezione viene affidata a **ntopng**, che monitora passivamente il traffico sulla porta 433 (HTTPS). Durante l'handshake del protocollo TLS (ovvero l'istante in cui client e server si accordano prima di cifrare i dati), lo strumento analizza l'estensione dedicata all'SNI. Se il client cerca di instaurare una comunicazione cifrata verso un servizio web omettendo tale parametro, ntopng intercetta immediatamente la violazione e genera un allarme chiamato `Missing SNI TLS Extn`.
...
* **Obiettivo**: Rilevare inversioni di ruolo sospette in cui un computer aziendale, che solitamente agisce come **client**, inizia improvvisamente ad accettare connessioni dall'esterno come se fosse un **server**. Questa metrica adotta una politica rigorosa, ad ogni attivazione di un servizio in ascolto su un nodo interno della rete viene considerata un'anomalia, sia essa dovuta a una backdoor malevola o a un'operazione tecnica non preventivata. Questo garantisce massima visibilità sui movimenti laterali e sull'uso di strumenti di amministrazione remota.
...
L'applicazione di modelli statistici classici per la *Network Anomaly Detection* sconta storicamente il limite della sensibilità agli *outlier*: i normali (e legittimi) picchi di traffico alterano la media aritmetica e la deviazione standard, generando cecità di falsi positivi.
...
1. **Immune all'inganno:** Basandosi su regole logiche fisse, il sistema non può essere manipolato o "avvelenato" dall'attaccante.
2. **Nessun riaddestramento:** Il modello non ha bisogno di imparare costantemente le nuove tattiche d'attacco, perché si concentra sul rilevare le violazioni dei principi di base della rete, che restano sempre invariati.
