...
def calcola_m_scan(client):
    ...
    righe = client.query(QUERY_M_SCAN).result_rows
...
def main():
    ...
    # Calcolo della metrica M_scan
    try:
        host_flaggati = calcola_m_scan(client)
    except Exception as e:
    ...
    if not host_flaggati:
        print("[OK] Nessun host flagged - nessuna attività di scan rilevata nell'ultima ora.\n")
        return
