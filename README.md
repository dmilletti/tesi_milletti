...
Per capire il grado di sospetto di un host $h$ in un dato istante viene utilizzato uno *Score globale* $S(h)$. Invece di limitarsi a un semplice approccio binario, il modello utilizza un sistema di **punteggio di rischio additivo** (da 0 a 100). L'equazione è una somma delle penalità assegnate per ogni anomalia rilevata, con un limite massimo:
...
Le metriche utilizzate sono suddivise in due classi: **deterministiche** e **statistiche**.
...
### 3.1: Analisi del comportamento atteso
...
### 3.2: Misurazione dello scostamento statistico (Z-Score robusto)
...
### 3.3: Rilevamento delle novità assolute (Novelty detection)
...
Definiamo $P_{storico}$ come l'insieme dei servizi che l'host usa abitualmente e definiamo $P_{oggi}$ come l'insieme dei servizi rilevati ed utilizzati oggi. La novità, che chiameremo ($N$) è calcolata prendendo la differenza fra questi due insiemi:
...
* **Caso A (segnale isolato):** Se l'host esegue unicamente una scansione di rete, si attiverà solo la relativa metrica. Lo *score* finale $S(h)$ il punteggio salirà solo in base ai punteggi previsti per quella specifica anomalia, mantenendo l'host in una fascia "verde", la quale non sarà considerata critica.
...
È proprio mettendo insieme queste piccole anomalie nella "zona grigia" che il sistema certifica il cambio di stato dell'host da "regolare" a "sospetto". Invece di cercare un'impronta digitale già nota, impara a riconoscere comportamenti sospetti anche quando sono nuovi e complessi.
...
3. **La densità delle anomalie:**  
   ...
   * **$|C| \gg 1$ :** diffusione sistemica o lateral movement in corso.
